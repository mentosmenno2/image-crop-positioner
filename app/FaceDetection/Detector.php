 ...
 * PHP face detection
 *
 * This face detector code is a direct port of  a JavaScript face detection library by Karthik Tharavaad.
 * @author Karthik Tharavaad <karthik_tharavaad@yahoo.com>
 *
 * This code has been ported to PHP by Maurice Svay.
...
class FaceDetector {
	...
	public function __construct( $detection_data = 'detection.dat' ) {
		...
		$this->detection_data = unserialize( file_get_contents( $detection_data ) ); // phpcs:ignore
	}

	/**
	 * @psalm-suppress all
	 */
	public function detect_faces( $file ) {
		if ( is_resource( $file ) ) {
		...
		if ( $ratio !== 0 ) {
			$reduced_canvas = imagecreatetruecolor( $im_width / $ratio, $im_height / $ratio );
			...
			$this->face = $this->do_detect_greedy_big_to_small(
				$stats['ii'],
				$stats['ii2'],
				$stats['width'],
				$stats['height']
	 ...
	 *
	 * @param string|null $out_file_name file name to store. If null, will be printed to output
	 *
	 * @throws NoFaceException
	 *
	 * @psalm-suppress all
	 */
	public function crop_face_to_jpeg( $out_file_name = null ) {
		if ( empty( $this->face ) ) {
		...
		imagejpeg( $canvas, $out_file_name );
	}
	...
	public function to_json() {
		return wp_json_encode( $this->face );
	}
	...
	protected function do_detect_greedy_big_to_small( $ii, $ii2, $width, $height ) {
		...
		for ( $scale = $start_scale; $scale > 1; $scale *= $scale_update ) {
			...
			$step     = max( $scale, 2 ) >> 0;
			$inv_area = 1 / ( $w * $w );
			for ( $y = 0; $y < $endy; $y += $step ) { // phpcs:ignore Generic.CodeAnalysis.JumbledIncrementer.Found
