...
class Crop {
	...
	public function crop( $payload, int $orig_w, int $orig_h, int $dest_w, int $dest_h, $crop ) {
		...
		// Get the spots bounding box
		$spots_bounding_box = $this->get_bounding_box( $spots );
		$spots_center       = $this->get_bounding_box_center( $spots_bounding_box );
	...
	protected function get_bounding_box_center( array $bounding_box ): array {
	 ...
	 * @return array
	 */
	protected function get_bounding_box( array $spots ): array {
		$bounding_box = array(
			'x_min' => PHP_INT_MAX,
			'x_max' => 0,
			'y_min' => PHP_INT_MAX,
